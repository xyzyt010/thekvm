...
|---|---|---|
| `kvm-daemon` | Windows: LocalSystem service · Linux: systemd system unit (`input` group) · FreeBSD: rc.d service | QUIC + UDP transports, pairing, input capture/injection incl. lock screen |
| `kvm-ui` | Logged-in user | Slint GUI: pairing UX, daemon status, settings |

## Transport

- **UDP, always** — there is no transport setting anywhere (no UI
  selector, no CLI switch). Pointer motion takes an encrypted
  low-latency fast path (ChaCha20Poly1305, key derived from the QUIC
  verify: both fingerprints + link epoch, no second pairing). No stream
  head-of-line blocking, so motion stays smooth even while keys,
  buttons, wheel, clipboard, and handoff share the link. Keys, buttons,
  wheel, clipboard, and handoff stay on the authenticated QUIC stream.
  Every motion packet falls back to QUIC automatically, so a blocked UDP
  port never breaks the link. The daemon always listens on both
  (`listen_port` for QUIC, `listen_port + 1` for UDP motion).

## Crates

- `kvm-core` — shared types, config, event model
- `kvm-protocol` — QUIC transport (quinn), UDP motion fast path,
  certificate-fingerprint pairing, wire messages
