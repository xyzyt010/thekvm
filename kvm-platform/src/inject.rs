...
mod linux_uinput {
    ...
    impl UinputDevice {
        ...
        /// gesture (see UinputTouch): the parked viewport-pinch experiment
        /// (see PINCH-ZOOM/README.md). Only used when the receiver opts in
        /// via `THEKVM_LINUX_PINCH_VIEWPORT=1`; the default rendering is
        /// Ctrl+wheel page zoom. `display` names the session X server for the anchor/dims
        ...
        fn warn_touch_once(&mut self, message: impl Into<String>) {
            if !self.touch_warned {
                ...
                tracing::warn!("{}; page zoom is the default rendering (viewport touch only under THEKVM_LINUX_PINCH_VIEWPORT=1, see PINCH-ZOOM/README.md)", message.into());
            }
        }
    ...
    //   `false` (warned once): the caller renders Ctrl+wheel page zoom
    //   by default and only reaches this touch path under the explicit
    //   `THEKVM_LINUX_PINCH_VIEWPORT=1` opt-in (parked work, see
    //   PINCH-ZOOM/README.md). A gesture never fails the send.
